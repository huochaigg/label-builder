...
function disableAll(win) {
  debug$1(`Disabling all shortcuts on window ${title(win)}`);
...
function enableAll(win) {
  debug$1(`Enabling all shortcuts on window ${title(win)}`);
  const wc = win.webContents;
  const shortcutsOfWindow = windowsWithShortcuts.get(wc);
  for (const shortcut of shortcutsOfWindow) {
...
function unregisterAll(win) {
  debug$1(`Unregistering all shortcuts on window ${title(win)}`);
  const wc = win.webContents;
  const shortcutsOfWindow = windowsWithShortcuts.get(wc);
...
function register(win, accelerator, callback) {
  ...
  if (Array.isArray(accelerator) === true) {
    accelerator.forEach((accelerator2) => {
      if (typeof accelerator2 === "string") {
        register(win, accelerator2, callback);
      }
    });
    return;
  }
  debug$1(`Registering callback for ${accelerator} on window ${title(win)}`);
  ...
  if (windowsWithShortcuts.has(wc)) {
    ...
    if (wc === ANY_WINDOW) {
      ...
      const enableAppShortcuts = (e, win2) => {
      ...
      windows.forEach((win2) => enableAppShortcuts(null, win2));
      app.on("browser-window-created", enableAppShortcuts);
      shortcutsOfWindow.removeListener = () => {
        const windows2 = BrowserWindow.getAllWindows();
        windows2.forEach(
          (win2) => win2.webContents.removeListener("before-input-event", keyHandler)
        );
...
function unregister(win, accelerator) {
  ...
  if (Array.isArray(accelerator) === true) {
    accelerator.forEach((accelerator2) => {
      if (typeof accelerator2 === "string") {
        unregister(win, accelerator2);
      }
    });
    return;
  }
  debug$1(`Unregistering callback for ${accelerator} on window ${title(win)}`);
  _checkAccelerator(accelerator);
  debug$1(`${accelerator} seems a valid shortcut sequence.`);
  if (!windowsWithShortcuts.has(wc)) {
...
function toggleDevelopmentTools(win = BrowserWindow$1.getFocusedWindow()) {
  if (win) {
...
async function registerAccelerators(win = BrowserWindow$1.getFocusedWindow()) {
...
function devTools(win = BrowserWindow$1.getFocusedWindow()) {
  if (win) {
    toggleDevelopmentTools(win);
  }
}
function openDevTools(win = BrowserWindow$1.getFocusedWindow()) {
  if (win) {
    win.webContents.openDevTools(developmentToolsOptions.get(win));
  }
}
function refresh(win = BrowserWindow$1.getFocusedWindow()) {
  if (win) {
    win.webContents.reloadIgnoringCache();
  }
}
function inspectElements() {
  const win = BrowserWindow$1.getFocusedWindow();
  const inspect = () => {
...
function debug(options) {
  ...
  app$1.on("browser-window-created", (event, win) => {
